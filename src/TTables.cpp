...
void tmodel<COUNT, PROB>::normalizeTable(const vcbList&, const vcbList&, int)
{
   ...
   for(unsigned int i=0;i<lexmat.size();++i){
       if( lexmat[i] ){
           ...
           for(unsigned int j=0;j<lSize;++j){
                  ...
                  WordIndex e=i,f=(*lexmat[i])[j].first;
                   (*lexmat[i])[j].second.prob = word2vec->L * word2vec->getW2VProb(e, f) + (1. - word2vec->L) * (*lexmat[i])[j].second.prob;
  ...
  cout << "probabilities combined with w2v probabilities with weight " << word2vec->L << ". " << endl;
  word2vec->L *= word2vec->LFactor;
