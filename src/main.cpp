...
GLOBAL_PARAMETER(float,L_w2v,"lw2v","weight of the w2v probabilities when combined with models probabilities",PARLEV_OPTHEUR,0.5);
GLOBAL_PARAMETER(float,LF_w2v,"lfw2v","the factor multiplied to L in each iteration of EM",PARLEV_OPTHEUR,1);
GLOBAL_PARAMETER(int,w2v_Method,"w2vMethod","how to integrate word2vec in model 1 (0: don't use w2v, "
                                            "1: just count similar words according to w2v n-best, "
                                            "2: use linear combination of w2v and models probabilities after each iteration)",PARLEV_OPTHEUR,0);
...
string Prefix, LogFilename, OPath, Usage, SourceVocabFilename,
       ...
       a_Filename, p0_Filename, d_Filename, n_Filename, dictionary_Filename, sourceWord2Vec_Filename, targetWord2Vec_Filename, tmat_Filename;
...
double StartTraining(int&result)
{
  ...
  Word2Vec *word2vec = new Word2Vec(w2v_Method, N_w2v, L_w2v, LF_w2v);
  if(useWord2Vec && w2v_Method){
        if(tmat_Filename != "")
      word2vec->readWord2VecWithTransmat(sourceWord2Vec_Filename, targetWord2Vec_Filename, tmat_Filename, &eTrainVcbList, &fTrainVcbList);
        else
        word2vec->readWord2Vec(sourceWord2Vec_Filename, targetWord2Vec_Filename, &eTrainVcbList, &fTrainVcbList);
  }
  int minIter=0;
  ...
  tmodel<COUNT, PROB> tTable(CoocurrenceFile, word2vec, useWord2Vec);
...
int main(int argc, char* argv[])
{
  ...
  getGlobalParSet().insert(new Parameter<string>(
                               "W2V_S",
                               ParameterChangedFlag,
                               "source word2vec file name",
                               sourceWord2Vec_Filename,PARLEV_INPUT));

  getGlobalParSet().insert(new Parameter<string>(
                               "W2V_T",
                               ParameterChangedFlag,
                               "target word2vec file name",
                               targetWord2Vec_Filename,PARLEV_INPUT));

  getGlobalParSet().insert(new Parameter<string>(
                               "tmat",
                               ParameterChangedFlag,
                               "translation matrix file name",
                               tmat_Filename,PARLEV_INPUT));
