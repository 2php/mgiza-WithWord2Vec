...
void* exe_emloop(void *arg)
{
  ...
  em->result = em->m1->em_thread(em->it,em->nthread,*em->dict,em->useDict, *em->word2vec, em->useWord2Vec);
  return arg;
}

int model1::em_thread(int noIterations, int nthread, /*Perplexity& perp, sentenceHandler& sHandler1, */
                      Dictionary& dictionary, bool useDict, Word2Vec& word2vec, bool useWord2Vec /*Perplexity* testPerp, sentenceHandler* testHandler,
...
//    dump_files = true;
  ...
  em_loop(it,perp, sHandler1, false, dump_files, alignfile.c_str(), dictionary, useDict, word2vec, useWord2Vec, trainViterbiPerp);
  return 0;
}
...
int model1::em_with_tricks(int noIterations, /*Perplexity& perp, sentenceHandler& sHandler1, */
                           ...
                           ,  Word2Vec& word2vec, bool useWord2Vec
                           /*Perplexity* testPerp, sentenceHandler* testHandler,
										     Perplexity& trainViterbiPerp, Perplexity* testViterbiPerp */
                           , bool dumpCount ,  const char* dumpCountName, bool useString)  // If specified, then will dump files before last iteration
{
  ...
  for(int it = 1; it <= noIterations; it++) {
    ...
    for (th=1; th<NCPUS; th++) {
      ...
      ths[th].word2vec = & word2vec;
      ths[th].useWord2Vec = useWord2Vec;
      ths[th].result = 0;
                     ...
                     bool dump_alignment, const char* alignfile, Dictionary& dict, bool useDict,  Word2Vec& word2vec, bool useWord2Vec, Perplexity& viterbi_perp, bool test)
{
  ...
  while(sHandler1.getNextSentence(sent)) {
    ...
    bool **isSimilar  = new bool*[m + 1];
    for(int _i = 0; _i < m+1; _i++){
      indict[_i] = new bool[l + 1];
      isSimilar[_i] = new bool[l + 1];
    }
#else
    bool eindict[l + 1];
    bool findict[m + 1];
